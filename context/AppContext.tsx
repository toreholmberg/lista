...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const listsSubscription = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'lists' }, loadData)
      .subscribe()

    const listItemsSubscription = supabase
      .channel('list_items_channel')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'list_items' }, loadData)
      .subscribe()
  ...
  const addList = async (name: string) => {
    try {
      ...
      const existingLists = lists.filter(list => 
      ...
      if (essentialItems.length > 0) {
        ...
        setListItems(currentListItems => [...currentListItems, ...(listItemsData || [])])
      }
    } catch (err) {
  ...
  const removeList = async (listId: string) => {
    try {
      ...
      setListItems(currentListItems => currentListItems.filter(li => li.list_id !== listId))
    } catch (err) {
  ...
  const removeItemCompletely = async (itemId: string) => {
    try {
      ...
      // Update local state
      setListItems(currentListItems => currentListItems.filter(li => li.item_id !== itemId))
